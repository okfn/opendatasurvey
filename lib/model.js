...
OpenDataCensus._loadCountrySubmissions = function (callback) {
  ...
  getCsvData(OpenDataCensus.data.countrysubmissions.resultsUrl, function(data) {
    ...
    var reviewersByPlace = [];
    var reviewers = _.reject(_.uniq(_.map(results, function(r) {
        if (reviewersByPlace[r['place']]) {
            if (!_.contains(reviewersByPlace[r['place']], r['reviewer'])) reviewersByPlace[r['place']].push(r['reviewer']);
        }
        else {
            reviewersByPlace[r['place']] = [];
            reviewersByPlace.push(r['reviewer']);
        }
        return r['reviewer'];
      })), function(val) {
      return (val==="");
    ...
    var submittersByPlace = [];
    var submitters = _.reject(_.uniq(_.map(results, function(r) {
        if (submittersByPlace[r['place']]) {
            if (!_.contains(submittersByPlace[r['place']], r['submitter'])) submittersByPlace[r['place']].push(r['submitter']);
        }
        else {
            submittersByPlace[r['place']] = [];
            submittersByPlace.push(r['submitter']);
        }
        return r['submitter'];
      })), function(val) {
      return (val==="");
    ...
    OpenDataCensus.data.countrysubmissions.submittersByPlace = submittersByPlace;
    OpenDataCensus.data.countrysubmissions.reviewersByPlace = reviewersByPlace;
