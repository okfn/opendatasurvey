...
OpenDataCensus.countryDatasetsMap = {
  ...
  OpenDataCensus.datasetNamesShortToLongMap = {
    'Transport Timetables' : 'Public Transport Timetables',
    'Government Budget' : 'Government Budget (National, high level, not detailed)',
    'Government Spending' : 'Government Spending (National, transactional level data)',
    'Election Results' : 'Election Results (national)',
    'Company Register' : 'Company Register',
    'National Map' : 'National Map (Low resolution: 1:250,000 or better)',
    'National Statistics' : 'National Statistical Data (economic and demographic information)',
    'Legislation' : 'Legislation (laws and statutes) - National',
    'Postcodes / Zipcodes' : 'National Postcode/ZIP database',
    'Emissions of pollutants' : 'Environmental Data on major sources of pollutants (e.g. location, emissions)'
 };
  
  OpenDataCensus.gUser = config.get('google:user');
  OpenDataCensus.gPass = config.get('google:password');
  
  OpenDataCensus.gKey = config.get('database:country:spreadsheetKey');
  
  //Not sure if this really belongs in here, but trying to process the config
  //in app.js leads to a timeout on Heroku
  OpenDataCensus.port = config.get('appconfig:port');

var openQuestions = OpenDataCensus.questions.slice(4,12);
...
var Backend = function(options) {
  ...
  this.options.reviewedSubmissionIndex = 2;
  this.options.entryIndex = 3;
...
Backend.prototype.getEntry = function(data, cb) {
  // TODO: require year/country/place
  this.get(this.options.entryIndex, data, cb);
};
...
Backend.prototype.getSubmission = function(data, cb) {
  this.get(this.options.submissionIndex, data, cb);
};
...
OpenDataCensus.load = function(cb) {
  ...
  OpenDataCensus.backend.login(function(err) {
    if (err) {
      throw Exception('Failed to login to Database (Google Spreadsheet)');
    }
    done();
  });
...
function cleanUpCommon(records) {
  ...
  var ynquestions = OpenDataCensus.questions.slice(4, 11);
