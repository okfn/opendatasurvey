...
OpenDataCensus.datasetNamesInverseMap = _.invert(OpenDataCensus.datasetNamesMap);
...
OpenDataCensus._loadCountrySubmissions = function (callback) {
  ...
  getCsvData(OpenDataCensus.data.countrysubmissions.resultsUrl, function(data) {
    ...
    var reviewers = _.sortBy(_.reject(_.uniq(_.map(results, function(r) {
        if (OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']]) {
            if (!_.contains(OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']], r['reviewer']) && r['reviewer'] !== "") OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']].push(r['reviewer']);
        }
        else if (r['reviewer'] !== "") {
            OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']] = [];
            OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']].push(r['reviewer']);
        }
        return r['reviewer'];
      })), function(val) {
      return (val==="");
    ...
    var submitters = _.sortBy(_.reject(_.uniq(_.map(results, function(r) {
        if (OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']]) {
            if (!_.contains(OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']], r['submitter']) && r['submitter'] !== "") OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']].push(r['submitter']);
        }
        else if (r['submitter'] !== "") {
            OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']] = [];
            OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']].push(r['submitter']);
        }
        return r['submitter'];
      })), function(val) {
      return (val==="");
...
function addUrlsToEntries (entries) {
  entries.forEach(function (entry) {
    ...
    var dataset = OpenDataCensus.datasetNamesInverseMap[entry.dataset];
