...
var Backend = function(options) {
  ...
  this.options.submissionsIndex = 1;
  this.options.entryIndex = 3;
  this.sheet = new GoogleSpreadsheet(options.key);
}
...
Backend.prototype.getEntry = function(data, cb) {
  ...
  this.getEntrys(data, function(err, rows) {
    if (err) {
      cb(err);
      return;
    }
...
Backend.prototype.getEntrys = function(query, cb) {
  ...
  this.sheet.getRows(this.options.entryIndex, {}, gquery, cb);
};


Backend.prototype.insertEntry = function(data, cb) {
  this.sheet.addRow(this.options.entryIndex, data, cb);
};
