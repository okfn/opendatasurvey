...
//Reviewers who didn't [get a chance to]
//put their name on the reviewer form
var extraReviewers = {};
extraReviewers['2013'] = ["Rasa Nedeljkov",
  "Masayuki Hatta",
  "Henri Laupmaa",
  "Larisa Rankovic",
  "Bojana Dzulovic",
...
OpenDataCensus.scoreOpenness = function(response) {
  var score=0;
  var tmp = {};
  _.each(OpenDataCensus.data.questions, function(q) {
    tmp[q.id] = q;
  });
  _.each(openQuestions, function(key) {
    //Cope with data from CSV functionality OR from Backend
    if (response[key]=='Y' || response[key]=='Yes') {
      score += parseInt(tmp[key].score);
    }
  });
  return score;
}
...
OpenDataCensus.load = function(cb) {
  ...
  // Order of operations for load:
  //
  // 1. Login
  // 2. Fetch questions (as other load procedures depend on the questions being
  //    loaded)
  // 3. Fetch everything else in parallel
  //
  Q.ninvoke(OpenDataCensus.backend, 'login')
  .then(function () { return Q.nfcall(OpenDataCensus._loadQuestions) })
  .then(function () {
    return Q.all([
      Q.nfcall(OpenDataCensus._loadCountryDatasets),
      Q.nfcall(OpenDataCensus._loadCountryResults),
      Q.nfcall(OpenDataCensus._loadCountrySubmissions)
    ]);
  })
  .nodeify(cb);
...
OpenDataCensus._loadQuestions = function (callback) {
  getCsvData(config.get('database:questionsUrl'), function(data) {
    OpenDataCensus.data.questions = data;
    callback();
  });
};

OpenDataCensus._loadCountryDatasets = function (callback) {
  getCsvData(config.get('database:country:datasetsUrl'), function(data) {
...
OpenDataCensus._loadCountryResults = function (callback) {
  getCsvData(config.get('database:country:resultsUrl'), function(data) {
    var results = cleanUpCountry(data);
    var country = OpenDataCensus.data.country;
    country.results = results;
    addUrlsToEntries(country.results);
    country.byplace = byPlace(results);
    country.bydataset = byDataset(results);
    country.places = _.uniq(_.map(results, function(r) {
      return r['place'];
    }));
    // Presort places by score, descending
    country.places.sort(function (a, b) {
      return country.byplace[b].score - country.byplace[a].score;
    });
    var summary = getSummaryData(results);
    summary.places = country.places.length;
...
OpenDataCensus._loadCountrySubmissions = function (callback) {
  ...
  getCsvData(OpenDataCensus.data.countrysubmissions.resultsUrl, function(data) {
    ...
    OpenDataCensus.data.countrysubmissions.reviewersByPlace = [];

    var thisYearsExtraReviewers = [];
    if (extraReviewers[OpenDataCensus.DEFAULT_YEAR]) thisYearsExtraReviewers = extraReviewers[OpenDataCensus.DEFAULT_YEAR];

    var reviewers = _.sortBy(_.union(_.reject(_.uniq(_.map(results, function(r) {
        if (OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']]) {
            if (!_.contains(OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']], r['reviewer'].trim()) && r['reviewer'].trim() !== "") OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']].push(r['reviewer'].trim());
        }
        else if (r['reviewer'].trim() !== "") {
            OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']] = [];
            OpenDataCensus.data.countrysubmissions.reviewersByPlace[r['place']].push(r['reviewer'].trim());
        }
        return r['reviewer'].trim();
      })), function(val) {
      return (val==="");
    }),thisYearsExtraReviewers), function (val) {return val;});

    OpenDataCensus.data.countrysubmissions.reviewers = reviewers;

    OpenDataCensus.data.countrysubmissions.submittersByPlace = [];
    var submitters = _.sortBy(_.reject(_.uniq(_.map(results, function(r) {
        if (OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']]) {
            if (!_.contains(OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']], r['submitter'].trim()) && r['submitter'].trim() !== "") OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']].push(r['submitter'].trim());
        }
        else if (r['submitter'].trim() !== "") {
            OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']] = [];
            OpenDataCensus.data.countrysubmissions.submittersByPlace[r['place']].push(r['submitter'].trim());
        }
    ...
    OpenDataCensus.data.countrysubmissions.submitters = submitters;
...
function cleanUpCommon(records) {
  ...
  var ynquestions = OpenDataCensus.openQuestions;
  var out = records.map(function(record) {
    ...
    record.ycount = OpenDataCensus.scoreOpenness(record);
...
function byDataset(data) {
  ...
  _.each(_.keys(OpenDataCensus.datasetNamesMap), function(datasetname) {
