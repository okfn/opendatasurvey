...
exports.parseSpreadsheetUrl = function(url) {
  var regex = /.*spreadsheet\/ccc?.*key=([^#?&+]+)[^#]*(#gid=([\d]+).*)?/,
    out = { key: '', sheet: 0, new_format: false }
      ;
  if (!url) {
    return out;
  }
  ...
  if (!matches) {
    regex = /.*spreadsheets\/d\/([^\/]+)\/[^#]*(#gid=([\d]+).*)?/
    matches = url.match(regex);
    out.new_format = !!matches;
  }
...
function _getCsvForGoogleSheet(info) {
  var tmpl = {
    old_format: 'https://docs.google.com/spreadsheet/pub?key=KEY&single=true&gid=INDEX&output=csv',
    new_format: 'https://docs.google.com/spreadsheets/d/KEY/export?single=true&gid=INDEX&format=csv'
  };
  return tmpl[info.new_format ? 'new_format' : 'old_format']
  ...
  exports.getCsvData(configCsvUrl, function(err, data) {
    ...
    if (!config.get('places')) {
      var places = _getCsvForGoogleSheet({
        ...
        new_format: configSheetInfo.new_format
      });
