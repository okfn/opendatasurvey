...
var cleanEntry = function(obj) {
  ...
  if (obj.timestamp && normalizedTimestamp.year() <= 2013) {
    obj.year = 2013;
...
utils.loadData({
  ...
  _.each(matches, function(m, i, l) {
    ...
    var submissionYears = _.map(m.submissions, function(s) {return s.year;});
    var matchYear;
    var candidates;
    var candidate;
    ...
    if (m.submissions.length === 0 ||
      ...
      // and, some have only matching future submissions
      // (which is actually the same)!
      // Looking at you global census, 2013 :)
      console.log('Entry has no matching submission');
      console.log(m.entry.year, m.entry.censusid,
        m.entry.place, m.entry.dataset);
      console.log('  ------  ');
      ...
      candidates = _.filter(m.submissions, function(s) {
        return s.year === matchYear;
      });
      candidate = _.first(_.sortBy(candidates, function(o) {
        return -o.createdAt;
      }));

      if (!candidate) {
        console.log('somehow we do not have a candidate match. ' +
  ...
  Promise.each(matches, function(m) {
    if (m.saveStrategy === 'create') {
      return models.Entry.create(m.toSave).then(function() {
        console.log('created');
      }).catch(console.trace.bind(console));
    } else {
      // update
      return m.toSave.save().then(function() {
        console.log('updated');
      }).catch(console.trace.bind(console));
    }
  });
