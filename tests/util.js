...
describe('Config - load', function(){
  ...
  it('config is loaded', function(done){
    ...
    assert.equal(config.get('questionsUrl'), 'https://docs.google.com/spreadsheet/pub?key=0Aon3JiuouxLUdEVHQ0c4RGlRWm9Gak54NGV0UlpfOGc&single=true&gid=1&output=csv');
    assert.equal(config.get('resultsUrl'), 'https://docs.google.com/spreadsheet/pub?key=0AqR8dXc6Ji4JdHR5WWdUU2dYUElPaFluUlBJbkFOMUE&single=true&gid=1&output=csv')
    done();
  });
});
